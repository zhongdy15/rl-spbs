...
tradeoff_constant_num = len(tradeoff_constant_list)

assert len(gpu_list) == tradeoff_constant_num, "gpu_setting is incorrect"

for tradeoff_constant_id in range(tradeoff_constant_num):
    for seed in seed_list:
        for algo_id in range(algo_num):
            tradeoff_constant = tradeoff_constant_list[tradeoff_constant_id]

            env_kwargs = f"reward_mode:\"'{reward_mode}'\" tradeoff_constant:{tradeoff_constant}"
            algo_filename = algo_list[algo_id] + "_" + reward_mode + \
                            "_" + str(tradeoff_constant) + \
                            "_" + time_flag
            cmd_line = f"CUDA_VISIBLE_DEVICES={gpu_list[tradeoff_constant_id]} " \
                       f"python train.py " \
