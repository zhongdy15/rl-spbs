...
save_folder = 'maskdqn_baseline_251119'
rl_baseline_251105 = {"ppo": "logs/ppo_Baseline_OCC_PPD_with_energy_10_2025-11-11-16-36-41/ppo/SemiPhysBuildingSim-v0_1",
                      ...
                      "dqn_mask": "logs/dqn_mask_Baseline_OCC_PPD_with_energy_10_2025-11-19-12-52-21/dqn_mask/SemiPhysBuildingSim-v0_1"}
...
for _ in range(1):
    ...
    while not done:
        ...
        action_array = action_index_to_array(int(action), [4, 4, 4, 4, 4, 4, 4])
        print("Step " + str(i) + " action:" + str(action) + " action_array:" + str(action_array))
        obs, r, done, info = env1.step(action)
        current_action_mask = info["action_mask"]
        # current_action_mask = current_action_mask.unsqueeze(0)
        current_action_mask = current_action_mask[np.newaxis, :]
        rewards += r
        if i==1:
