...
# --- Constants ---
# 注意：SYSTEM_PROMPT 和 INSTRUCTION_PROMPT 的内容将从上述更新的文件中读取。
...
OUTPUT_DIR = "all_types_sft_data_v5"
...
if len(common_timestamps) > 1:
    for i in range(1, len(common_timestamps)):
        ...
        # --- 将以下原始的 output_json 生成逻辑替换掉 ---
        # 原代码中的 output_json 生成逻辑 (约 100 行)：
        # output_rooms = []
        # output_reasons = []
        ...
        #         outdoor_desc = ""
        #         if current_outdoor_temp > OPTIMAL_TEMP_MAX + 1:
        ...
        #             if curr_fcu > 0 or (curr_fcu != prev_fcu):
        ...
        #             elif curr_fcu > 0 and prev_fcu == 0:
        ...
        for room_id in ROOM_IDS:
            ...
            confidence_scores_list.append({"room_id": room_id, "confidence_to_adjust": confidence})

        output_str = json.dumps(confidence_scores_list, indent=2, ensure_ascii=False)
